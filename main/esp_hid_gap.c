...
// Vendored near-verbatim from ESP-IDF's examples/bluetooth/esp_hid_device
// (main/esp_hid_gap.c) - generic NimBLE/Bluedroid GAP+bonding boilerplate,
// not specific to that example's own HID report maps/demo tasks. Only the
// NimBLE code paths (CONFIG_BT_NIMBLE_ENABLED) are actually compiled in
// this project (see main/CMakeLists.txt/sdkconfig.defaults - Classic
// BT/Bluedroid are never enabled, since ESP32-S3 has no BR/EDR anyway).
// The one deliberate change from upstream is the security config in
// esp_hid_ble_gap_adv_init() below (Just Works instead of passkey-entry
// pairing - see the comment there). See
...
// The HID service UUID never changes between calls - a plain static
// avoids esp_hid_ble_gap_adv_init()'s previous per-call malloc() (see
// below), which leaked sizeof(ble_uuid16_t) bytes every single
// ble_hid_device_start() (this function is called once per start(), and
// `fields` being static meant each call's malloc() overwrote
// fields.uuids16 with a fresh pointer, permanently losing the previous
// cycle's allocation). Small per-cycle (~4-8 bytes), so unlikely to be
// the whole story behind the ENOMEM/assert crash this was found while
// investigating (see mds/usb_hid/2026-09-13_ble_idle_crash.md), but a
// real, confirmed leak in its own right regardless - fixed outright by
// not allocating at all.
static const ble_uuid16_t s_hid_svc_uuid16 = BLE_UUID16_INIT(GATT_SVR_SVC_HID_UUID);
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    fields.uuids16 = &s_hid_svc_uuid16;
    fields.num_uuids16 = 1;
    fields.uuids16_is_complete = 1;

    /* Initialize the security configuration.
     *
     * Changed from the upstream example's BLE_SM_IO_CAP_DISP_ONLY +
     * sm_mitm=1 (which assumes the device can display a passkey for the
     * peer to type in): this board has no display or input, so that
     * combination would leave pairing stuck expecting passkey entry that
     * can never happen. NO_IO + mitm=0 selects Just Works pairing
     * instead (no PIN/passkey at all) - see
     * mds/usb_hid/2026-09-07_ble_hid_sink_plan.md's ペアリング section:
     * pairing is meant to be driven entirely from the target PC's own
     * Bluetooth settings UI, with no UI on this board at all. */
    ble_hs_cfg.sm_io_cap = BLE_SM_IO_CAP_NO_IO;
    ble_hs_cfg.sm_bonding = 1;
    ble_hs_cfg.sm_mitm = 0;
    // sm_sc (LE Secure Connections, ECDH P-256-derived LTK) was
    // temporarily disabled while chasing the reconnect-after-reset
    // encryption-failure loop (mds/usb_hid/2026-09-07_ble_hid_sink_impl.md's
    // follow-up) - real-hardware testing showed no difference either way,
    // which makes sense in hindsight: the actual root cause turned out to
    // be CONFIG_BT_NIMBLE_NVS_PERSIST defaulting to n (see
    // sdkconfig.defaults), so NimBLE's bond store was RAM-only and lost
    // every bond on reset regardless of SC vs Legacy pairing. Left at the
    // normal/more-secure default now that that's fixed.
    ble_hs_cfg.sm_sc = 1;
...
esp_err_t esp_hid_gap_init(uint8_t mode)
{
    ...
    // NimBLE's own MODLOG_DFLT()/BLE_HS_LOG() calls (this file's own
    // BLE_GAP_EVENT_NOTIFY_TX/BLE_GAP_EVENT_MTU handlers below, plus
    // ble_att_svr.c and friends deep in the host stack) all log under the
    // fixed tag "NimBLE" at esp_log's ordinary runtime severity - see
    // modlog.h's MODLOG_ESP_LOCAL(): its own compile-time gate
    // (BLE_HS_LOG_LVL <= LOG_LOCAL_LEVEL) only ever disables a whole
    // severity tier at build time, it does *not* consult
    // CONFIG_BT_NIMBLE_LOG_LEVEL per call the way it looks like it would -
    // the actual print-or-not decision for an already-compiled-in call is
    // just esp_log's normal per-tag runtime level, which defaults to
    // CONFIG_LOG_DEFAULT_LEVEL (INFO) regardless of that Kconfig choice.
    // So bumping CONFIG_BT_NIMBLE_LOG_LEVEL to WARNING (sdkconfig.defaults)
    // alone didn't silence "notify_tx event" (fires on every HID
    // notification while BLE is in use - console noise, and likely
    // latency too given blocking UART output has already been measured
    // as a real cost on this project's other hot paths - see
    // mds/usb_hid/2026-08-24_rp2040_bridge_fps_investigation.md). Setting
    // the "NimBLE" tag's runtime level directly is what actually gates
    // it, without touching CONFIG_LOG_DEFAULT_LEVEL (which would lower
    // every *other* tag's verbosity too).
