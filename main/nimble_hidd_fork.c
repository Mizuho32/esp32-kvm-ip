/*
 * Fork of ESP-IDF's esp_hid component, src/nimble_hidd.c (BLE HID device
 * profile backend), with one behavioral change - see kvm_ble_hidd_dev_init()
 * and nimble_hid_stop_gatts() below, and
 * mds/usb_hid/2026-09-11_ble_reconnect_holdoff.md's follow-up for why this
 * fork exists at all.
 *
 * Everything else in this file is an unmodified copy of ESP-IDF's original
 * (esp-idf commit matching this checkout's /opt/esp-idf) - resist the urge
 * to "clean up" or restyle it, since that just makes future diffing against
 * upstream harder. The two non-static externally-linked symbols the
 * original exposed (esp_ble_hidd_dev_init, nimble_host_reset) are made
 * `static` here purely to avoid a duplicate-symbol link error against the
 * *original*, unmodified nimble_hidd.c object file that's still compiled as
 * part of idf::esp_hid (excluding it from that build was considered and
 * rejected - CONFIG_BT_NIMBLE_HID_SERVICE, the only Kconfig gating it, also
 * gates ble_svc_hid.c itself, which this file - and this project's mruby
 * System Control / HID sinks - still need). This project only ever talks to
 * *this* copy, through kvm_ble_hidd_dev_init() (nimble_hidd_fork.h) instead
 * of the generic esp_hidd_dev_init(..., ESP_HID_TRANSPORT_BLE, ...); once
 * that returns, the resulting esp_hidd_dev_t behaves identically to one
 * from the original (same dev->deinit/input_set/... function pointers,
 * just pointing at this file's copies) - esp_hidd_dev_input_set() and
 * esp_hidd_dev_deinit() work completely unchanged, no other call site
 * anywhere needs to know this fork exists.
 */
...
static const char *TAG = "NIMBLE_HIDD_FORK";
...
static void nimble_host_reset(int reason); // static here - see file header comment
...
static int nimble_hid_stop_gatts(esp_ble_hidd_dev_t *dev)
{
    ...
    // NOT ble_gatts_stop() here anymore (upstream has it right at this
    // point) - this project's ble_hid_device.c always follows this whole
    // deinit up with a full esp_hid_gap_deinit() (esp_nimble_deinit() ->
    // ble_hs_deinit()), which calls the exact same generic ble_gatts_stop()
    // itself as part of a *complete* NimBLE host teardown. Calling it here
    // *too* meant it ran twice per stop cycle - crashed on real hardware
    // (LoadProhibited inside ble_gatts_free_mem(), see
    // mds/usb_hid/2026-09-11_ble_reconnect_holdoff.md's follow-up for the
    // addr2line-resolved trace). None of the ble_svc_*_deinit() calls below
    // depend on ble_gatts_stop() having already run - each frees its own
    // service definitions independently (ble_gatts_free_svcs(), itself
    // idempotent/NULL-guarded) - so skipping it here and letting
    // ble_hs_deinit() be the sole caller is safe, and correct as long as
    // this fork is only ever used the way ble_hid_device.c uses it (always
    // paired with a subsequent esp_hid_gap_deinit()). A caller that used
    // this fork *without* ever calling esp_nimble_deinit() afterward would
    // leak the GATT database - not this project's usage pattern, but worth
    // knowing if this file is ever reused elsewhere.
...
static esp_err_t esp_ble_hidd_dev_init(esp_hidd_dev_t *dev_p, const esp_hid_device_config_t *config, esp_event_handler_t callback)
{
    int rc;
...
// Mirrors esp_hidd.c's own esp_hidd_dev_init()'s ESP_HID_TRANSPORT_BLE case
// (calloc the outer esp_hidd_dev_t, delegate to the transport-specific
// init, stamp ->transport) - this project calls this directly instead of
// the generic esp_hidd_dev_init(..., ESP_HID_TRANSPORT_BLE, ...) so that
// the transport-specific init that runs is *this* file's (renamed-static)
// esp_ble_hidd_dev_init() above, not the original nimble_hidd.c's. Once
// this returns, the esp_hidd_dev_t behaves identically either way -
// esp_hidd_dev_input_set()/esp_hidd_dev_deinit() etc. from esp_hidd.c work
// unchanged, since they just follow the function pointers this file's own
// esp_ble_hidd_dev_init() wired up above.
esp_err_t kvm_ble_hidd_dev_init(const esp_hid_device_config_t *config, esp_event_handler_t callback, esp_hidd_dev_t **dev_out)
{
    ...
    esp_err_t ret = esp_ble_hidd_dev_init(dev, config, callback);
    if (ret != ESP_OK) {
        free(dev);
        return ret;
    }
